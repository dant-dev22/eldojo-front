...
const staggerDelay = (index: number, total: number, startPct: number, endPct: number) => {
  ...
  return [
    ...
    [before / 1, 0],
    [at / 1, 1],
    [Math.min(1, at + windowPct * 0.45) / 1, 1],
    [1, 1],
  ];
};

export const AnimatedLogoHero: React.FC<HeroProps> = ({ testID }) => {
  const { width: viewportWidth } = useWindowDimensions();

  const logoSize = viewportWidth < 480
    ? 140
    : viewportWidth < 768
    ? 180
    : viewportWidth < 1100
  ...
  const headlineSize = viewportWidth < 480
    ? 24
    : viewportWidth < 768
    ? 30
    : viewportWidth < 1100
    ...
    return (
      <View style={styles.root} testID={testID}>
        ...
        >
          ...
          >
            <defs>
              <linearGradient id="alh-sealGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#C62828" />
                <stop offset="45%" stopColor="#A81F1F" />
                <stop offset="100%" stopColor="#8B1717" />
              </linearGradient>
              <linearGradient id="alh-goldGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#F9A825" />
                <stop offset="55%" stopColor="#FBC02D" />
                <stop offset="100%" stopColor="#D98E1A" />
              </linearGradient>
              <radialGradient id="alh-innerShine" cx="35%" cy="30%" r="70%">
            ...
            <MotionG
              style={{ transformOrigin: "500px 460px", transformBox: "fill-box" }}
              animate={{
                scale: [
                  0.85, 0.85,
                  1, 1,
            ...
            >
              ...
              <MotionCircle
                ...
                cy={outerCy}
                r={outer}
                fill="url(#alh-innerShine)"
                initial={{ opacity: 0 }}
                animate={{
                  opacity: [0, 0, 0.7, 0.9, 0.7],
                }}
                transition={{
                  ...BASE_TRANSITION,
              ...
              <MotionCircle
                ...
                animate={{
                  strokeDashoffset: [
                    ...
                    0,
                    0,
                    2 * Math.PI * (outer - 22) * 0.0,
                  ],
                  opacity: [0, 0, 0.9, 0.9, 0.9],
                }}
                transition={{
                  ...BASE_TRANSITION,
                ...
                {[
                  ...
                  return (
                    <MotionPath
                      ...
                      strokeLinecap="round"
                      fill="none"
                      initial={{ pathLength: 0, opacity: 0, x: i % 2 === 0 ? -10 : 10, y: i < 2 ? -10 : 10 }}
                      animate={{
              ...
              >
                {KANJI_PATHS.map((d, i) => {
                  const pct = staggerDelay(i, KANJI_PATHS.length, 0.20, 0.60);
                  return (
                    <MotionPath
                      key={`kanji-${i}`}
                      d={d}
                      ...
                      transition={{
                        ...BASE_TRANSITION,
                        times: pct.map((p) => p[0]).length === 5 ? undefined : pct.map((p) => p[0]),
                      }}
                    />
                  );
                })}
              ...
              <MotionG fill="url(#alh-goldGrad)">
                {[
                  ...
                  const pct = staggerDelay(i, 4, 0.58, 0.68);
                  return (
                    <MotionCircle
                      key={`dot-${i}`}
                      cx={c.cx}
                      ...
                      animate={{
                        opacity: pct.map((p) => (p[1] > 0 ? 0.9 : 0)),
                        scale: [0, 0, 1, 1.35, 1, 1],
                      }}
                      transition={{
                        ...BASE_TRANSITION,
                        times: [0, 0.55, 0.64, 0.70, 0.78, 1],
                      }}
                    />
                  );
                })}
        ...
        <Text
          style={[
            styles.headline,
            {
              ...
              maxWidth: viewportWidth < 768 ? viewportWidth - spacing.lg * 2 : 820,
...
const NativeAnimatedLogo: React.FC<NativeProps> = ({ logoSize, headlineSize, viewportWidth, testID }) => {
  const overallScale = useRef(new NativeAnimated.Value(0.85)).current;
  ...
  const corner = useRef(KANJI_PATHS.slice(0, 4).map(() => new NativeAnimated.Value(0))).current;
  const cornerTransl = useRef(KANJI_PATHS.slice(0, 4).map(() => new NativeAnimated.Value(10))).current;
  ...
  const buildLoop = () => {
    ...
    const step = (ms: number) =>
      NativeAnimated.timing(new NativeAnimated.Value(0), {
        toValue: 1,
        duration: 1,
    ...
    const scaleSettle = NativeAnimated.timing(overallScale, {
      toValue: 1,
      duration: pct(240),
      easing: NativeEasing.out(NativeEasing.back(1.6)),
      useNativeDriver: false,
    });
    ...
    const reset = NativeAnimated.parallel([
      ...
      NativeAnimated.timing(sealOpacity, {
        toValue: 0,
        duration: 1,
        useNativeDriver: false,
      }),
      NativeAnimated.timing(shineOpacity, {
        toValue: 0,
        duration: 1,
        useNativeDriver: false,
      }),
      NativeAnimated.timing(borderOffset, {
        toValue: 2 * Math.PI * 358,
        duration: 1,
        useNativeDriver: false,
      }),
      NativeAnimated.timing(borderOpacity, {
      ...
      >
        ...
        >
          <Svg width="100%" height="100%" viewBox="0 0 1000 1000">
            <SvgDefs>
              <SvgLinearGradient id="alh-sealGrad-native" x1="0%" y1="0%" x2="100%" y2="100%">
                <SvgStop offset="0%" stopColor="#C62828" />
                <SvgStop offset="45%" stopColor="#A81F1F" />
                <SvgStop offset="100%" stopColor="#8B1717" />
              </SvgLinearGradient>
              <SvgLinearGradient id="alh-goldGrad-native" x1="0%" y1="0%" x2="100%" y2="100%">
                <SvgStop offset="0%" stopColor="#F9A825" />
                <SvgStop offset="55%" stopColor="#FBC02D" />
                <SvgStop offset="100%" stopColor="#D98E1A" />
              </SvgLinearGradient>
              <AnimatedRadial id="alh-innerShine-native" cx="35%" cy="30%" r="70%">
                <SvgStop offset="0%" stopColor="#FFFFFF" stopOpacity={NativeAnimated.multiply(shineOpacity, 0.10)} />
            ...
            <AnimatedCircle
              ...
              cy={outerCy}
              r={outer}
              fill="url(#alh-innerShine-native)"
            ...
            <AnimatedCircle
              ...
              r={outer - 22}
              fill="none"
              stroke="url(#alh-goldGrad-native)"
              strokeWidth={3.5}
              strokeDasharray={2 * Math.PI * (outer - 22)}
            ...
            <SvgG fill="url(#alh-goldGrad-native)">
              {[
                { cx: 500, cy: outerCy - outer + 8 },
                { cx: 500, cy: outerCy + outer - 8 },
                { cx: outerCx - outer + 8, cy: outerCy },
                ...
                <AnimatedG
                  key={`dot-native-${i}`}
                  transform={[{ scale: dotScale[i] }, { translateX: c.cx }, { translateY: c.cy }]}
                >
