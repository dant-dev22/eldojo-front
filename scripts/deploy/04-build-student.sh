...
# Build student: usa script package.json build:web:student (cross-env + expo export --output-dir dist-student/)
...
log "Build student via npm run build:web:student → $DIST_STUDENT ..."
npm run --silent build:web:student >> "$LOG_FILE" 2>&1
